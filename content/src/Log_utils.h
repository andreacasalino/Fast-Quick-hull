...
void Log_CovexHull(const std::list<V>& cloud, const std::string& log_file, const size_t& Iterations  = 0) {
	...
	for (size_t k = 0; k < N_facets; k++) 
		f << incidences[3 * k] << " " << incidences[3 * k + 1] << " " << incidences[3 * k + 2] << std::endl;
	//deallocate the incidences buffer
	free(incidences);
...
void Log_as_JSON_CovexHull(const std::list<V>& cloud, const std::string& log_file, const size_t& Iterations = 0) {
	...
	//put the above info in a textual file for displaying it, using Result_visualization/Render.html
