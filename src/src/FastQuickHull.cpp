...
void QuickHullSolver::_convexHull(std::vector<FacetIncidences> &indices,
  ...
  std::pair<std::array<hull::Coordinate, 4>, std::array<std::size_t, 4>>
      tethraedron = hndlr.getInitialTethraedron();
  hull::Hull hull(tethraedron.first[0], tethraedron.first[1],
                  tethraedron.first[2], tethraedron.first[3], obs.get());
  std::map<const hull::Coordinate *, std::size_t> indiceMap;
  {
  ...
  const std::map<const hull::Facet *, std::pair<int, float>> *distMap =
      &obs->getDistanceMap();
  std::map<const hull::Facet *, std::pair<int, float>>::const_iterator farthest,
      itF;
  ...
  for (std::size_t iter = 0; iter < this->maxIterations; ++iter) {
    ...
    hull.UpdateHull(hndlr.getCoordinate(farthest->second.first),
                    *farthest->first);
